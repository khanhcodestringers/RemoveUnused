...
public class SSSceneManager : MonoBehaviour 
{
	...
	/// <summary>
	/// Destroies the current stack without destroy scenes that created.
	/// Create new stack that has the first scene is current scece
	/// </summary>
	/// <returns>The current stack without destroy scene.</returns>
	public string DestroyCurrentStackWithoutDestroyScene()
	{
		Stack<string> stack = m_CurrentStackScreen;
		string s = null;
    ...
    public void ShowLoading(float alpha = 0.6f, float timeOut = 0, NoParamCallback callBack = null, float delay = 0.5f)
	{
    ...
    private void SetShieldColor(GameObject shield, Color color)
    {
        switch (m_UIType)
        {
            ...
            case UIType.nGUI:
                ...
                if (color.a > 0)//them
                   mesh.material.DOFade(color.a, .5f).Play();//them
                break;
            default:
                break;
        }
	...
	protected void ShieldOff()
	{
        ...
        if (m_ListShield.Count >= i)
        {
            ...
            m_ListShield[i].GetComponentInChildren<MeshRenderer>().material.DOFade(3f / 255f, .1f).Play().OnComplete(() => //them
            {
                m_ListShield[i].SetActive(false);//them
            });//them
        }
    ...
    protected void SetCanvases(string sn, float i, bool shouldChangeCanvasOrder = true)
    {
        GameObject sc = m_DictAllScene[sn];

        SetCanvases (sc, i, shouldChangeCanvasOrder);
    }

    protected void SetCanvases(GameObject sc, float i, bool shouldChangeCanvasOrder = true)
    {
		if(!shouldChangeCanvasOrder){return;}
		...
		LoadOrActive (sn, () => 
			{
				...
				// On Set
				SSController ct = GetController(sn);
				SetCanvases(sn, ic, ct.ShouldChangeCanvasOrder);
				if (ct != null)
				{
					ct.OnSet(data);
				}
